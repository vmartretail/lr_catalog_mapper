...
st.set_page_config(page_title="Marketplace Catalog Mapper", layout="wide")
st.markdown(
    """
    <style>
...
st.sidebar.info(
    """
1. Upload a CSV or Excel file with your marketplace catalog.
2. Select the corresponding marketplace.
3. Review (and optionally edit) the mapping table below.
4. Check the box if you want to proceed even if some expected headers are missing.
5. Click **Convert Files** to transform and download the catalog in LimeRoad format.
6. You can save your mapping changes or reset to default using the buttons below.
    """
)
...
# Load mapping from JSON if it exists, otherwise use the default mapping
...
def transform_catalog(df: pd.DataFrame, marketplace: str) -> (pd.DataFrame, list):
    ...
    for lr_field, mapping in mapping_dict.items():
        ...
        if marketplace_field:  # Only process non-empty mappings
...
# Section to display and edit mapping
...
# st.subheader("Edit Mapping (if needed)")
...
# Main file upload and processing section
...
# Checkbox for proceeding even if expected headers are missing
